...
static struct user *create_user(char *name, enum user_type type, union user_data data)
{
    ...
    if (name == NULL) {
        ...
        if ((user->name = (char *) malloc(strlen(name) + 1)) == NULL) {
            free(user);
            return NULL;
        }
    ...
    if (type == STUDENT) {
        ...
        create_instructor(data.instructor.salary, instructor);
...
{
    if (push_front(list, name, type, data) == FAILURE) {
        return FAILURE;
    }
    list->tail = list->head;
...
{
    if (list == NULL || index < 0 || index > list->size - 1 || dataOut == NULL) {
        return FAILURE;
    }
    if (index == 0) {
        return pop_front(list, dataOut);
...
int get_highest_paid(struct user_list *list, struct user **dataOut)
{
    ...
    for (int i = 0; i < list->size; i++) {
        if(current->type == INSTRUCTOR) {
          found = 1;
          if(current->data.instructor.salary > max) {
            max = current->data.instructor.salary;
            *dataOut = current;
          }
        }
        current = current->next;

...
static metadata_t *find_right(metadata_t *freed_block) {
    ...
    if (address_list == NULL || address_list->next == NULL) {
        return NULL;
    }

    uint8_t *ret = (uint8_t *)freed_block + freed_block->size + TOTAL_METADATA_SIZE ;
    metadata_t *curr = address_list;
    while (curr <= freed_block && curr != NULL) {
        curr = curr->next;
    }
    if ((uint8_t *)curr == ret && curr != NULL) {
        return (metadata_t *)ret;
    } else {
        return NULL;
    }
    //return nextB != NULL && (uint8_t *)nextB == retptr ? (metadata_t *)retptr : NULL;
...
static metadata_t *split_block(metadata_t *block, size_t size) {
    uint8_t *newB = (uint8_t *)block + (block->size - size);
    metadata_t *ptr = (metadata_t *)newB;
...
void *my_malloc(size_t size) {
    ...
    if (!block) {
        while (!block) {
            metadata_t *t = my_sbrk(SBRK_SIZE);
            if ((uint8_t *)-1 == (uint8_t *)t) {
                my_malloc_errno = OUT_OF_MEMORY;
                return NULL;
            }
            t->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
            t->next = NULL;
            add_to_addr_list(t);
            if (find_left(t)) {
                merge(find_left(t), t);
...
void my_free(void *ptr) {
    ...
    uint8_t *block = (uint8_t *)ptr - TOTAL_METADATA_SIZE;
    metadata_t *newBlock = (metadata_t *)block;
    add_to_addr_list(newBlock);
    metadata_t *temp = find_left(newBlock);
    if (temp) {
        merge(temp, newBlock);
        if (find_right(temp)) {
            merge(temp, find_right(temp));
        }
    } else{
        if (find_right(newBlock)) {
            merge(newBlock, find_right(newBlock));
        }
    }
...
void *my_calloc(size_t nmemb, size_t size) {
    ...
    metadata_t *ptr = my_malloc(totSize);
    if (ptr) {
        return memset(ptr, 0, totSize);
    } else {
        return NULL;
    }
